...
describe('useAutosave', () => {
  ...
  describe('debounced saves', () => {
    it('debounces saves (~800ms)', () => {
      renderHook(({ c }) => useAutosave('note-id', c), { initialProps: { c: 'initial' } });
      expect(mockUpdateNote).not.toHaveBeenCalled();
      act(() => { vi.advanceTimersByTime(500); });
      expect(mockUpdateNote).not.toHaveBeenCalled();
      act(() => { vi.advanceTimersByTime(400); });
      expect(mockUpdateNote).toHaveBeenCalledWith('note-id', 'initial');
    });

    it('resets debounce timer on content change', () => {
      const { rerender } = renderHook(
        ({ c }) => useAutosave('note-id', c),
        { initialProps: { c: 'a' } }
      );
      act(() => { vi.advanceTimersByTime(400); });
      rerender({ c: 'ab' });
      ...
      expect(mockUpdateNote).toHaveBeenCalledWith('note-id', 'abc');
    });
    ...
    it('only saves once for rapid changes', () => {
      ...
      expect(mockUpdateNote).toHaveBeenCalledWith('note-id', 'e');
    });
  });

  describe('API calls', () => {
    it('calls API to update note content', () => {
      renderHook(() => useAutosave('note-id', 'test content'));
      act(() => { vi.advanceTimersByTime(800); });
      expect(mockUpdateNote).toHaveBeenCalledWith('note-id', 'test content');
    });
  ...
  describe('save status', () => {
    it('returns status "saving" while save is in progress', () => {
      ...
      const { result } = renderHook(() => useAutosave('note-id', 'content'));
      act(() => { vi.advanceTimersByTime(800); });
  ...
  describe('edge cases', () => {
    it('does not save empty content', () => {
      renderHook(() => useAutosave('note-id', ''));
      act(() => { vi.advanceTimersByTime(800); });
      expect(mockUpdateNote).not.toHaveBeenCalled();
    });
    ...
    it('does not save when content exceeds 100KB', () => {
      ...
      renderHook(() => useAutosave('note-id', oversized));
      act(() => { vi.advanceTimersByTime(800); });
      expect(mockUpdateNote).not.toHaveBeenCalled();
    });
  });
